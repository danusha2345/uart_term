...
- **Система декодеров:** Четыре режима фрейминга — **Raw** (без разбиения), **Delimiter** (по байтовому разделителю, например `B5 62`), **SLIP** (RFC 1055) и **COBS**. Каждое соединение (UART1, UART2, BLE) выбирает свой декодер независимо.
