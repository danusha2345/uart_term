...
- **Система декодеров:** Пять режимов фрейминга — **Raw** (без разбиения), **Delimiter** (по байтовому разделителю), **SLIP** (RFC 1055), **COBS** и **NMEA** (`$...CRLF`). Каждое соединение (UART1, UART2, BLE) выбирает свой декодер независимо.
...
- **Метки протоколов:** UBX-пакеты подписываются по классу/ID, NMEA-предложения — по talker/sentence (например, `[GN-RMC]`). Ошибка checksum помечается как `[CRC!]`.
...
2. **NMEA:** Выберите декодер `NMEA` — терминал будет собирать строки от `$` до `LF`, убирать `CR/LF`, подписывать тип предложения и проверять checksum после `*XX`.
3. **Другие текстовые протоколы:** Установите разделитель `"$"` или `\n` в режиме `Delimiter`.
4. **SLIP/COBS:** Выберите соответствующий декодер в выпадающем списке — разделитель не требуется.
5. **Неизвестный baud rate:** Нажмите "Auto" — терминал переберёт стандартные скорости и выберет оптимальную.
6. **Беспроводной UART (BLE):** Переключитесь в режим `BLE`, нажмите "Scan" и выберите устройство. NUS-характеристики определяются автоматически.
7. **Копирование данных:** Правый клик по пакету — копирование payload в HEX или ASCII.
